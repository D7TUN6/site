  ...
  for (const dirent of dirents) {
    ...
    try {
      ...
      if (coverImage) {
        ...
        try {
          await access(path.join(SHOP_ROOT, slug, 'images', previewFile))
          coverPreviewUrl = `/media/shop/${slug}/images/${previewFile}`
        } catch {
          coverPreviewUrl = `/media/shop/${slug}/images/${coverImage}`
        }
      }
      products.push({
        slug,

...
function parseOptions(body: Record<string, unknown>): DownloadOptions {
  const format = typeof body.format === 'string' ? body.format : 'flac'
  return {
    format,
    sampleRate: typeof body.sampleRate === 'number' ? body.sampleRate : 44100,
    bitDepth: (typeof body.bitDepth === 'number' ? body.bitDepth : 16) as number,
    channels: (typeof body.channels === 'number' ? body.channels : 2) as number,
    resampler: (typeof body.resampler === 'string' ? body.resampler : 'none') as string,
    bitrateMode: (typeof body.bitrateMode === 'string' ? body.bitrateMode : 'vbr') as string,

...
export function createYooKassaRouter({ db, hub }: { db: DatabaseSync; hub: OrderHub }) {
  ...
  router.post('/create', enforceSameOrigin, requireUser, async (req, res) => {
    ...
    if (expectedTotal !== order.items_total) return res.status(400).json({ error: 'Order total mismatch' })
  ...
  router.post('/webhook', async (req, res) => {
    ...
    if (!paymentId || eventType !== 'payment.waiting_for_capture' && eventType !== 'payment.succeeded') return res.status(200).json({ ok: true })
    ...
    if (minor !== null && minor !== expectedAmount) return res.status(200).json({ ok: true })

...
import {
  ROOT, SHOP_ROOT, SHOP_IMG_EXT, SHOP_CONVERT_EXTS, shopSlugify,
...
export function createAdminShopRouter({ db }: { db: DatabaseSync }) {
  ...
  router.delete('/:slug', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!targetDir.startsWith(SHOP_ROOT + path.sep)) return res.status(400).json({ error: 'Invalid slug' })
  ...
  router.delete('/:slug/images/:filename', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!imagesDir.startsWith(SHOP_ROOT + path.sep)) return res.status(400).json({ error: 'Invalid slug' })
      const targetFile = path.resolve(imagesDir, filename)
      if (!targetFile.startsWith(imagesDir + path.sep)) return res.status(400).json({ error: 'Invalid filename' })

import { mkdir, readdir, readFile, rename, rm, writeFile, access, symlink } from 'node:fs/promises'
...
import { processCoverImage, convertAudioToHls, spawnRebuild, exists, IMAGE_CONVERT_EXTS } from '../../lib/media-convert.js'
...
async function readManifest() {
  try {
    ...
    const manifest = JSON.parse(raw) as { releases?: Array<{ slug: string; tracks?: Array<{ sourceUrl?: string; streamUrl?: string; title?: string }>; coverUrl?: string; coverPreviewUrl?: string; releaseDate?: string; releaseType?: string }> }
...
async function writeReleaseMdx(slug: string, albumDir: string) {
  ...
  let notes = ''
  try { notes = await readFile(path.join(albumDir, 'notes', 'notes'), 'utf-8') } catch {}
...
export function createAdminReleasesRouter() {
  ...
  router.get('/', requireAdmin, async (_req, res) => {
    try {
      ...
      const releases = await Promise.all(dirents.filter((d) => d.isDirectory()).map(async (d) => {
        ...
        if (!releaseDate) {
          try { releaseDate = (await readFile(path.join(MUSIC_ROOT, d.name, '.release-date'), 'utf-8')).trim() || null } catch {}
        }
        let releaseType: string | null = mRelease?.releaseType ?? null
        if (!releaseType) {
          try { releaseType = (await readFile(path.join(MUSIC_ROOT, d.name, '.release-type'), 'utf-8')).trim() || null } catch {}
        }
        ...
        let isHidden = false
        try { const h = await readFile(path.join(MUSIC_ROOT, d.name, '.release-hidden'), 'utf-8'); isHidden = h.trim() === 'true' } catch {}
        return {
  ...
  router.post('/', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    if (!contentType.includes('multipart/form-data')) {
      ...
      if (!slug) return res.status(400).json({ error: 'Invalid album name' })
      const newDir = path.join(MUSIC_ROOT, albumName)
      try { await access(newDir); return res.status(409).json({ error: 'Release with this name already exists' }) } catch { /* ok */ }
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        bb.on('file', (field: string, stream: NodeJS.ReadableStream, info: { filename: string }) => {
          if (field === 'cover') {
            ...
            const ext = path.extname(info.filename).toLowerCase()
            if (!AUDIO_EXTS.has(ext)) { stream.resume(); return }
      ...
      if (!RELEASE_TYPES.includes(releaseType)) releaseType = 'album'
  ...
  router.post('/:slug/cover', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!savedFile) return res.status(400).json({ error: 'No valid image file received' })
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        bb.on('file', (_field: string, stream: NodeJS.ReadableStream, info: { filename: string }) => {
          ...
          const ws = createWriteStream(dest)
          stream.pipe(ws)
          pending.push(new Promise<void>((r, j) => { ws.on('finish', r); ws.on('error', j); stream.on('error', j) }))
        })
        bb.on('error', reject)
        bb.on('finish', () => Promise.all(pending).then(() => resolve()).catch(reject))
        req.pipe(bb)
      ...
      if (!saved.length) return res.status(400).json({ error: 'No valid audio files received' })
...
async function cleanupTempFiles(root: string) {
  try {
    ...
    for (const d of dirs) {
      ...
      try {
        ...
        if (changed) console.log(`[admin] cleaned up temp files in ${d.name}/tracks/source/`)

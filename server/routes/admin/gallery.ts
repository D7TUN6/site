import { mkdir, readdir, readFile, writeFile, rm, rename, access } from 'node:fs/promises'
...
import { processGalleryImage, spawnRebuild, exists, IMAGE_CONVERT_EXTS } from '../../lib/media-convert.js'
...
export function createAdminGalleryRouter() {
  ...
  router.get('/', requireAdmin, async (_req, res) => {
    try {
      ...
      for (const d of dirents) {
        ...
        for (const f of imgFiles) {
          ...
          if (GALLERY_IMG_EXT.has(path.extname(f).toLowerCase()) && !f.includes('-preview.')) {
            if (!cover) cover = `/media/gallery/${d.name}/${f}`
            images.push(f)
          }
        }
        let mdxMeta = { title: d.name, date: '', tags: [] as string[] }
        try {
          ...
          if (tagsM) mdxMeta.tags = tagsM[1].split(',').map((s) => s.trim().replace(/^"|"$/g, '')).filter(Boolean)
        } catch { /* mdx optional */ }
      ...
      return res.status(200).json({ ok: true, entries: applyOrder(entries as Array<{ slug: string }>, GALLERY_ROOT, galleryOrder) as typeof entries })
    } catch (err) {
  ...
  router.post('/', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    const dir = path.join(GALLERY_ROOT, slug)
    try { await access(dir); return res.status(409).json({ error: 'Entry with this slug already exists' }) } catch { /* ok */ }
  ...
  router.post('/:slug/images', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    try { await access(entryDir) } catch { return res.status(404).json({ error: 'Entry not found' }) }
    ...
    try {
      ...
      let mdx = { title: slug, date: '', tags: [] as string[], cover: '', images: [] as string[] }
      try {
  ...
  router.patch('/:slug', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    if (typeof req.body?.title === 'string') mdx.title = req.body.title.trim()
    if (typeof req.body?.date === 'string') mdx.date = req.body.date.trim()
    if (Array.isArray(req.body?.tags)) mdx.tags = req.body.tags.map(String)

    const imgFiles = await readdir(entryDir)
    const allImages = imgFiles.filter((f) => GALLERY_IMG_EXT.has(path.extname(f).toLowerCase()) && !f.includes('-preview.'))
    if (Array.isArray(req.body?.images)) {
      const existingSet = new Set(allImages)
      mdx.images = req.body.images.filter((f: unknown) => typeof f === 'string' && existingSet.has(f))
    } else {
  ...
  router.delete('/:slug/images/:filename', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    if (!targetFile.startsWith(GALLERY_ROOT + path.sep)) return res.status(400).json({ error: 'Invalid path' })

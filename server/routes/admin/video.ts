...
import { convertVideoToHls, generateVideoThumbnail, spawnRebuild, exists } from '../../lib/media-convert.js'
...
async function writeMdx(slug: string, data: { title: string; date: string; thumbnail: string; sources: Array<{ url: string; type: string; resolution?: string }> }) {
...
export function createAdminVideoRouter() {
  ...
  router.get('/', requireAdmin, async (_req, res) => {
    try {
      ...
      for (const d of dirents) {
        ...
        let mdxMeta = { title: d.name, date: '', thumbnail: '', sources: [] as Array<{ url: string; type: string; resolution?: string }> }
        try {
      ...
      return res.status(200).json({ ok: true, entries: applyOrder(entries as Array<{ slug: string }>, VIDEO_ROOT, videoOrder) as typeof entries })
    } catch (err) {
  ...
  router.post('/:slug/upload', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!savedFile) return res.status(400).json({ error: 'No valid video file received' })

      const srcPath = path.join(videoDir, savedFile)
      const { playlist, thumbnail } = await convertVideoToHls(srcPath, videoDir, savedFile)

      let mdx = { title: slug, date: '', thumbnail: '', sources: [] as Array<{ url: string; type: string; resolution?: string }> }
      try {
  ...
  router.patch('/:slug', enforceSameOrigin, requireAdmin, async (req, res) => {
    ...
    try {
      ...
      if (titleM) mdx.title = titleM[1]
      if (dateM) mdx.date = dateM[1]
      if (thumbM) mdx.thumbnail = thumbM[1]
    } catch { /* ok */ }

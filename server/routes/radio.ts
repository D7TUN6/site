...
type RadioScheduleSlot = { day: string; start: string; end: string; label: string }
type RadioState = {
  isLive: boolean
  listeners: number
...
let schedule: RadioScheduleSlot[] = []
let lastRegeneratedAt: string | null = null
...
async function loadSchedule() {
  try {
    ...
    schedule = JSON.parse(raw) as RadioScheduleSlot[]
  } catch {
    schedule = []
  }
}
...
export async function isStreamStale(manifestPath: string): Promise<boolean> {
  try {
    // validate content: must be non-empty and valid HLS header
    const raw = await readFile(STREAM_FILE, { encoding: 'utf8', flag: 'r' })
    if (raw.length < 20 || !raw.startsWith('#EXTM3U')) return true
...
export async function rebuildM3u8FromSegments(): Promise<boolean> {
  try {
    ...
    for (const s of segs) m3u8 += '#EXTINF:10.0000,\n' + s + '\n'
    m3u8 += '#EXT-X-ENDLIST\n'
...
function getBroadcastPosition(): number {
  if (totalDuration <= 0 || regeneratedAtEpoch <= 0) return 0
...
export function startPeriodicRegeneration(manifestPath: string) {
  setInterval(async () => {
    ...
    if (!(await exists(STREAM_FILE))) return
    try {
      const streamStat = await stat(STREAM_FILE)
      const manifestStat = await stat(manifestPath)
      if (manifestStat.mtimeMs <= streamStat.mtimeMs) return
    } catch { return }
    isRegenerating = true
    try {
...
export async function isSafeToRegenerate(): Promise<boolean> {
  return activeListeners.size === 0
}

export async function regenerateRadioStream(manifestPath: string) {
  const releases = await readManifest(manifestPath)
  const sourceFiles: string[] = []
  const trackInfos: { name: string; album: string; coverUrl: string }[] = []
  ...
  sourceFiles.length = 0; trackInfos.length = 0
...
export async function loadTimeline() {
  try {
    ...
    if (Array.isArray(catalog.timeline)) currentTimeline = catalog.timeline
    if (typeof catalog.totalDuration === 'number') totalDuration = catalog.totalDuration
    if (typeof catalog.regeneratedAtEpoch === 'number') regeneratedAtEpoch = catalog.regeneratedAtEpoch
    if (typeof catalog.generatedAt === 'string') lastRegeneratedAt = catalog.generatedAt
  } catch { /* ok */ }
}
...
export function createRadioRouter({ manifestPath }: { manifestPath: string }) {
  ...
  router.get('/state', async (_req, res) => {
    ...
    let trackCount = 0
    let regeneratedAt: string | null = lastRegeneratedAt
    try {
  ...
  router.get('/segments/:segment', (req, res) => {
    ...
    res.sendFile(segmentPath, (err) => {
      if (err) {
        if (!res.headersSent) res.status(404).json({ error: 'Segment not found' })
      }
    })
  })
  ...
  router.get('/tracks', async (_req, res) => {
    try {
      const raw = await readFile(path.join(RADIO_DIR, '.catalog.json'), 'utf-8')
      const catalog = JSON.parse(raw) as { tracks: string[] }
  ...
  router.get('/:segment', (req, res, next) => {
    ...
    if (!/^segment_\d{3}\.ts$/.test(name)) return next()

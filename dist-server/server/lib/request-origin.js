...
function firstHeaderValue(value) { return typeof value === 'string' ? value.split(',')[0].trim() : ''; }
...
function isLoopbackHost(hostname) {
    ...
    if (value === '::1')
        return true;
    if (/^127\.\d+\.\d+\.\d+$/.test(value))
        return true;
    if (/^192\.168\.\d+\.\d+$/.test(value))
        return true;
    if (/^10\.\d+\.\d+\.\d+$/.test(value))
        return true;
    if (/^172\.(1[6-9]|2\d|3[0-1])\.\d+\.\d+$/.test(value))
        return true;
    return false;
}
function hostFromNormalizedOrigin(origin) {
...
export function enforceSameOrigin(req, res, next) {
    ...
    if (sourceOrigin) {
        ...
        if (appOrigin && sourceOrigin === appOrigin)
            return next();
        ...
        if (!isProduction() && expectedOrigin) {
            ...
            if (isLoopbackHost(expectedHost) && isLoopbackHost(sourceHost))
                return next();
        }

    ...
    return new Promise((resolve, reject) => {
        ...
        ff.on('close', (code) => {
            if (code === 0)
                ...
                reject(new Error(Buffer.concat(err).toString('utf8') || `ffprobe exit ${code}`));
        });
    });
}
export const IMAGE_CONVERT_EXTS = new Set(['.jpg', '.jpeg', '.png', '.tiff', '.bmp']);
export async function processGalleryImage(src, destDir) {
...
export async function processCoverImage(src, destDir) {
    const ext = path.extname(src).toLowerCase();
    const base = path.basename(src, ext);
    const webpFilename = `${base}.webp`;
    const previewFilename = `${base}-preview.webp`;
...
export async function convertVideoToHls(src, destDir, filename) {
...
export async function convertAudioToHls(src, destDir, filename, keepSource = true) {
    const stem = path.basename(filename, path.extname(filename));
    const hlsDir = path.join(destDir, 'hls');
    await mkdir(hlsDir, { recursive: true });
    const playlistPath = path.join(hlsDir, 'index.m3u8');
...
export async function generateVideoThumbnail(src, destDir) {
    const ext = path.extname(src).toLowerCase();
    const base = path.basename(src, ext);
...
function pcmCodec(depth, le) {
    if (depth === 8)
        return 'pcm_u8';
    if (depth === 16)
        return le ? 'pcm_s16le' : 'pcm_s16be';
    if (depth === 24)
        return le ? 'pcm_s24le' : 'pcm_s24be';
    if (depth === 32)
        return le ? 'pcm_s32le' : 'pcm_s32be';
...
export function cacheKeyFromOpts(opts) {
...
export async function convertAudioToFormat(src, destDir, stem, opts) {
    ...
    const args = ['-y', '-i', src, '-map_metadata', '-1', '-vn', '-sn', '-dn'];
...
export function spawnRebuild() {
    ...
    rebuildTimeout = setTimeout(() => {
        ...
        let genOutput = '';
        generator.stdout.on('data', (c) => { genOutput += c.toString(); });
        generator.stderr.on('data', (c) => { genOutput += c.toString(); });
        generator.on('close', (genCode) => {
            if (genCode !== 0)
                console.error('Release manifest generation failed:', genOutput);
            else
                console.log('Release manifest regenerated:', genOutput.trim());
        });

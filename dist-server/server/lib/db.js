...
function defaultDbPath(rootDir) {
    return path.join(rootDir, 'server', 'generated', 'app.db');
}
export function openAppDb({ rootDir }) {
...
function migrate(db) {
  ...
  `);
    ...
    if (!columns.has('banned'))
        db.exec("ALTER TABLE users ADD COLUMN banned INTEGER NOT NULL DEFAULT 0");
    if (!columns.has('banned_at'))
        db.exec("ALTER TABLE users ADD COLUMN banned_at INTEGER");
    if (!columns.has('updated_at_real'))
        db.exec("ALTER TABLE users ADD COLUMN updated_at_real INTEGER");
}

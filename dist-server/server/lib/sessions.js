...
function cookieBaseOptions() {
    ...
    return { httpOnly: true, sameSite: 'lax', secure: isProduction(), path: '/', ...(domain ? { domain } : {}) };
}
function makeToken() { return crypto.randomBytes(32).toString('base64url'); }
function hashToken(token) { return crypto.createHmac('sha256', getAppSecret()).update(token).digest('hex'); }
function nowMs() { return Date.now(); }
export function setUserSessionCookie(res, token) { res.cookie(USER_SESSION_COOKIE, token, { ...cookieBaseOptions(), maxAge: USER_SESSION_TTL_MS }); }
export function clearUserSessionCookie(res) { res.cookie(USER_SESSION_COOKIE, '', { ...cookieBaseOptions(), maxAge: 0 }); }
export function setAdminSessionCookie(res, token) { res.cookie(ADMIN_SESSION_COOKIE, token, { ...cookieBaseOptions(), maxAge: ADMIN_SESSION_TTL_MS }); }
export function clearAdminSessionCookie(res) { res.cookie(ADMIN_SESSION_COOKIE, '', { ...cookieBaseOptions(), maxAge: 0 }); }
...
export function getUserBySessionToken(db, token) {
    ...
    const tokenHash = hashToken(token);
    const now = nowMs();
    const row = db.prepare(`SELECT users.id as id, users.email as email, users.email_verified as email_verified FROM sessions INNER JOIN users ON users.id = sessions.user_id WHERE sessions.token_hash = ? AND sessions.expires_at > ? LIMIT 1`).get(tokenHash, now);
    if (!row)
        return null;
...
export function revokeUserSession(db, token) { if (!token)
    return; db.prepare('DELETE FROM sessions WHERE token_hash = ?').run(hashToken(token)); }
export function revokeAdminSession(db, token) { if (!token)
    return; db.prepare('DELETE FROM admin_sessions WHERE token_hash = ?').run(hashToken(token)); }
export function cleanupExpiredSessions(db) { const now = nowMs(); db.prepare('DELETE FROM sessions WHERE expires_at <= ?').run(now); db.prepare('DELETE FROM admin_sessions WHERE expires_at <= ?').run(now); }

...
export function createAdminUsersRouter({ db }) {
    ...
    router.get('/', requireAdmin, (_req, res) => {
        try {
            const rows = db.prepare('SELECT id, email, email_verified, banned, banned_at, created_at, updated_at FROM users ORDER BY id DESC').all();
    ...
    router.patch('/:id', enforceSameOrigin, requireAdmin, (req, res) => {
        try {
            ...
            const existing = db.prepare('SELECT id, email, banned FROM users WHERE id = ?').get(id);
            if (!existing)

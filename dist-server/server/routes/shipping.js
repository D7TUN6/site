...
export function createShippingRouter() {
    ...
    router.get('/pickup-points', async (req, res) => {
        ...
        const r = await fetch(`https://search-maps.yandex.ru/v1/?${params}`);
        if (!r.ok)
            return res.status(502).json({ error: 'Pickup points lookup failed' });
        ...
        const points = features.map((f) => {
            ...
            const id = typeof p?.CompanyMetaData?.id === 'string' ? p.CompanyMetaData.id : `pt_${Math.abs((name + address).split('').reduce((a, ch) => ((a << 5) - a) + ch.charCodeAt(0), 0)).toString(16)}`;

...
function safeParseJson(raw) { try {
...
export function createOrdersRouter({ db, hub }) {
    ...
    router.get('/mine', requireUser, (req, res) => {
        ...
        return res.json({ ok: true, orders: rows.map((row) => ({
                id: row.id,
                status: row.status,
    ...
    router.get('/:orderId', requireUser, (req, res) => {
        ...
        const items = db.prepare('SELECT product_slug,product_title,unit_price,quantity FROM order_items WHERE order_id = ? ORDER BY id ASC').all(orderId);
        const events = db.prepare('SELECT id,kind,message,data_json,created_at FROM order_events WHERE order_id = ? ORDER BY id ASC').all(orderId);
    ...
    router.post('/', enforceSameOrigin, requireUser, (req, res) => {
        ...
        try {
            ...
            for (const item of norm)
                insertItem.run(orderId, item.slug, item.title, item.unitPrice, item.quantity);

...
export async function regenerateRadioStream(manifestPath) {
    ...
    await runFfmpeg([
        ...
        '-hls_segment_filename', path.join(SEGMENTS_DIR, 'segment_%03d.ts'),
        STREAM_FILE,
    ]);
...
`, 'utf-8');
        ...
        console.error('radio index.mdx write failed', e);
    }
...
export function createRadioRouter({ manifestPath }) {
    ...
    router.get('/stream', async (_req, res) => {
        // never block: serve existing stream immediately
        if (await exists(STREAM_FILE)) {
            // trigger background regeneration if stale (fire-and-forget)
            if (!isRegenerating && await isStreamStale(manifestPath)) {
                isRegenerating = true;
                regenerateRadioStream(manifestPath)
                    .then(() => { isRegenerating = false; return loadTimeline(); })
                    .catch((err) => { isRegenerating = false; console.error('radio bg regeneration failed', err); });
            }
            res.sendFile(STREAM_FILE);
            return;
        }
        // no stream at all — trigger first-time generation
        if (!isRegenerating) {
            isRegenerating = true;
            regenerateRadioStream(manifestPath)
                .then(() => { isRegenerating = false; return loadTimeline(); })
                .catch((err) => { isRegenerating = false; console.error('radio first-time generation failed', err); });
        }
        res.status(503).type('text/plain')
            .send('Radio stream is being generated — please try again in a few minutes.');
    });
    ...
    // simulate continuous broadcast: time since regeneration, wrapped by total duration
    router.get('/broadcast2', async (_req, res) => {
        const position = getBroadcastPosition();
        console.error('BROADCAST-HANDLER: lastRegeneratedAt=', lastRegeneratedAt); let generatedAt = lastRegeneratedAt;
        try {
            const raw = await readFile(CATALOG_FILE, 'utf-8');
            const catalog = JSON.parse(raw);
            if (typeof catalog.generatedAt === 'string')

...
function safeJsonStringify(value) { return JSON.stringify(value ?? null); }
function toMinorUnits(amountValue) { const m = amountValue.trim().match(/^(\d+)(?:\.(\d{1,2}))?$/); if (!m)
    return null; return Number(m[1]) * 100 + Number(String(m[2] || '0').padEnd(2, '0')); }
function verifyWebhookSignature(req) {
    ...
    if (!match)
        return false;
    const expected = Buffer.from(`:${secretKey}`).toString('base64');
    const actual = Buffer.from(match[1]);
    if (actual.length !== expected.length)
        return false;
    return crypto.timingSafeEqual(actual, Buffer.from(expected));
}
export function createYooKassaRouter({ db, hub }) {
    ...
    router.post('/create', enforceSameOrigin, requireUser, async (req, res) => {
        ...
        const recalc = db.prepare('SELECT SUM(unit_price * quantity) as total FROM order_items WHERE order_id = ?').get(order.id);
    ...
    router.post('/webhook', async (req, res) => {
        ...
        const eventType = typeof req.body?.event === 'string' ? req.body.event : '';

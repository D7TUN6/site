...
import { mkdir, readdir, rename, rm, writeFile, readFile, access } from 'node:fs/promises';
...
const SHOP_CONVERT_EXTS = new Set(['.jpg', '.jpeg', '.png']);
const SHOP_ROOT = path.join(ROOT, 'public', 'media', 'shop');
function slugify(value) { return value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').replace(/--+/g, '-'); }
...
function shopSlugify(value) { return value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').replace(/--+/g, '-'); }
function normalizeParam(value, pattern) {
    if (typeof value !== 'string')
        return '';
    return value.replace(pattern, '');
}
function nowMs() { return Date.now(); }
function safeParseJson(text) { if (typeof text !== 'string' || !text)
    return null; try {
...
function safeJsonStringify(value, maxLen = 8000) { const t = JSON.stringify(value ?? null); if (t.length > maxLen)
    throw new Error('Payload too large'); return t; }
...
async function processShopImage(src, destDir) {
...
async function removeShopImageFiles(imagesDir, filename) {
...
export function createAdminRouter({ db }) {
    ...
    router.patch('/orders/:orderId', enforceSameOrigin, requireAdmin, (req, res) => {
        ...
        if (pickupPoint != null) {
            ...
            params.push(safeJsonStringify(pickupPoint));
        }
        if (!updates.length)
    ...
    router.post('/orders/mock', enforceSameOrigin, requireAdmin, (req, res) => {
        const now = nowMs();
        const adminEmail = normalizeEmail(requireEnv('ADMIN_EMAIL'));
        let user = db.prepare('SELECT id, email FROM users WHERE email = ? LIMIT 1').get(adminEmail);
        if (!user) {
        ...
        try {
            ...
            if (Array.isArray(trackDeletes)) {
                ...
                for (const filename of trackDeletes) {
                    ...
                    if (!p.startsWith(tracksDir + path.sep) && p !== tracksDir)
                        continue;
            ...
            if (trackRenames) {
                ...
                for (const [oldName, newName] of Object.entries(trackRenames)) {
                    ...
                    if (!oldPath.startsWith(tracksDir + path.sep) || !newPath.startsWith(tracksDir + path.sep))
                        continue;

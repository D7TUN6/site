...
export async function downloadRelease(release: ReleaseEntry, opts: DownloadFormatArgs) {
  const payload = { slug: release.slug, ...opts }
  if (isLikelyIOSDevice()) return submitHiddenPost('/api/releases/download', payload as unknown as Record<string, string>)
  ...
  anchor.download = parseDownloadFileName(response.headers.get('content-disposition'), `${release.slug}-${opts.format}.zip`)
  document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(objectUrl), 0)
}

export async function downloadTrack(release: ReleaseEntry, trackIndex: number, opts: DownloadFormatArgs) {
  const payload = { slug: release.slug, track: String(trackIndex), ...opts }
  if (isLikelyIOSDevice()) return submitHiddenPost('/api/releases/track', payload as unknown as Record<string, string>)
  ...
  const anchor = document.createElement('a')
  anchor.href = objectUrl
  anchor.download = parseDownloadFileName(response.headers.get('content-disposition'), `${release.slug}-${trackIndex}.${opts.format}`)
  document.body.appendChild(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(objectUrl), 0)
}

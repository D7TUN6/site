...
export type YMapsPlacemark = {
  events: { add: (eventName: string, cb: () => void) => void }
}
...
export type YMapsApi = {
  ...
  Map: new (el: HTMLElement, options: { center: [number, number]; zoom: number; controls?: string[] }) => YMapsMap
  Placemark: new (coords: [number, number], props: { balloonContent: string }, options: { preset?: string }) => YMapsPlacemark
}

declare global {
  interface Window { ymaps?: YMapsApi }
}
...
export function loadYandexMaps(apiKey: string, lang: 'ru_RU' | 'en_US' = 'ru_RU'): Promise<YMapsApi> {
  ...
  if (window.ymaps) return new Promise((resolve) => window.ymaps?.ready(() => resolve(window.ymaps as YMapsApi)))
  ...
  if (!loaderPromise) {
    loaderPromise = new Promise<YMapsApi>((resolve, reject) => {
      ...
      script.onload = () => {
        if (!window.ymaps) return reject(new Error('Yandex Maps failed to load'))

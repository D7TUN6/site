...
const password = ref("");
const code = ref("");
const status = ref<"idle" | "loading" | "needs_code" | "error">("idle");
const message = ref("");
...
const showAdminHint = computed(() => {
  ...
  return text.includes("not verified") || text.includes("не подтверж");
});

const adminHintText = computed(() =>
  props.lang === "ru"
    ? "Если вы админ, входите через админ-страницу."
    : "If you're the admin, sign in via the admin page."
);
...
async function onRegister() {
  ...
  try {
    ...
    status.value = "needs_code";
    message.value = props.lang === "ru" ? "Код отправлен на почту." : "Code was sent to your email.";
  } catch (error) {
    status.value = "error";
...
async function onVerify() {
  ...
  message.value = "";
  try {
    await auth.verify({ email: email.value, code: code.value, lang: props.lang });
    status.value = "idle";
    await loadOrders();
  } catch (error) {
    status.value = "error";
    message.value = error instanceof Error ? error.message : "Unable to verify";
  }
}
...
async function onResend() {
  ...
  message.value = "";
  try {
    await auth.resend({ email: email.value, lang: props.lang });
    status.value = "needs_code";
    message.value = props.lang === "ru" ? "Код отправлен повторно." : "Code sent again.";
  } catch (error) {
    status.value = "error";
    message.value = error instanceof Error ? error.message : "Unable to resend";
  }
}
  ...
  <div v-else class="auth">
    ...
    <div class="auth-form">
      ...
      <div class="auth-actions">
        ...
        <button v-if="status === 'needs_code'" type="button" class="shop-btn shop-btn-secondary" :disabled="isLoading" @click="onResend">
          {{ lang === "ru" ? "отправить ещё раз" : "resend" }}
        </button>
      </div>

      <div v-if="status === 'needs_code'" class="auth-code">
        <label class="form-field">
          <span class="form-label">{{ lang === "ru" ? "код" : "code" }}</span>
          <input v-model="code" class="form-input" inputmode="numeric" />
        </label>
        <button type="button" class="shop-btn" :disabled="isLoading" @click="onVerify">
          {{ lang === "ru" ? "подтвердить" : "verify" }}
        </button>
      </div>

      <p v-if="message" class="checkout-hint">{{ message }}</p>
      <p v-if="showAdminHint" class="checkout-hint">
        {{ adminHintText }}
        <RouterLink :to="adminHref">{{ lang === "ru" ? "админ" : "admin" }}</RouterLink>
      </p>
    </div>
  </div>
</template>

    ...
    config = {
      ...
      systemd.services.d7tun6-site = {
        ...
        serviceConfig = {
          ...
          ExecStart = "${pkgs.bash}/bin/bash -c '${pkgs.nodejs_24}/bin/npm ci --include=dev --no-audit --no-fund && ${pkgs.nodejs_24}/bin/npm run build && exec ${pkgs.nodejs_24}/bin/node /var/www/d7tun6.site/server/index.mjs'";
